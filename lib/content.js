...
export function normalizeProjectEntry(entry) {
  ...
  const title = entry.data.title ?? extractMarkdownTitle(entry.rawContent) ?? humanizeSlug(sourceSlug);
  ...
  return {
    ...
    slug,
    title,
    summary: entry.data.summary ?? extractMarkdownSummary(entry.rawContent),
    topic: "projects",
